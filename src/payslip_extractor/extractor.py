...
def scan_pdfs(
    ...
    logger.info("Scanning %d PDF file(s)...", total_pdfs)
    ...
    for pdf_index, pdf_path in enumerate(pdf_list, 1):
        ...
        pdf_matches = sum(1 for m in matches if m.source_pdf == pdf_path)
        if found_in_pdf:
            logger.info(
                "[%d/%d] %s - done, %d match(es) in %.1fs",
                pdf_index,
                total_pdfs,
                pdf_path.name,
                pdf_matches,
